...
import { getOpenAIClient, regenerateContentBlock } from "@/lib/ai";
...
export async function POST(
  ...
  try {
    ...
    const regenerableTypes = ["text", "heading", "key_insight", "key_point"];
    if (!regenerableTypes.includes(block.type)) {
      return NextResponse.json(
        { error: `Cannot regenerate block type: ${block.type}` },
        { status: 400 }
      );
    }
    ...
    const result = await regenerateContentBlock(client, {
      ...
      currentContent: block.content as Record<string, unknown>,
      pageContent,
    ...
    const updated = await prisma.contentBlock.update({
      where: { id: blockId },
      data: { content: result.content as Prisma.InputJsonValue },
    });
